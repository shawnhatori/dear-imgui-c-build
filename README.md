...
compiled into a C-compatible static library. Unique static libraries are created
for the Win32 platform backend with each of the DX11, DX12, and Vulkan graphics
API backends, as well as debug and production (i.e. `-MTd` vs `-MT`).
...
the `.h` headers to include and the `.lib` static library to link against in a C
application.
